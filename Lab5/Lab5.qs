...
namespace Lab5 {
    ...
    operation Exercise2 (
       ...
       // if you phase flip twice you get the original phase back
       // if its odd parity phase is permanently flipped so just cz twice
    ...
    operation Exercise3 (
        ...
        use (input, target) = (Qubit[inputLength],Qubit()){
            //prepare target and input register
            X(target);
            ApplyToEach(H,input);
            //apply oracle
            oracle(input,target);
            //hadamard
            ApplyToEach(H,input);
            //if any output is a 1 return false
            for i in input{
                if M(i) == One{
                    set retval = false;
                }
            }
        //have to reset target before returning for Q# to not complain
        X(target);
        return retval;
        }
    }
